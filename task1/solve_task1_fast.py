"""
Task 1: ChEBI Ontology - Fast approach using sklearn SGD classifiers
Memory-efficient: trains one classifier at a time, no large model objects kept
"""
...
def smiles_to_fp(smiles_list):
    n = len(smiles_list)
    # ECFP4 2048-bit + MACCS 167-bit = 2215 features
...
def main():
    t0 = time.time()
    print("Task 1: ChEBI Multi-Label Classification (SGD)")
    ...
    print(f"Train: {len(train_df)}, Test: {len(test_df)}, Classes: {len(label_cols)}")
    ...
    for i in range(len(label_cols)):
        ...
        weight = {0: 1.0, 1: neg / max(pos, 1)}
        clf = SGDClassifier(
            loss='modified_huber',  # gives probabilities
            class_weight=weight,
            max_iter=100,
            random_state=42,
            n_jobs=1,
        ...
        if proba.shape[1] == 2:
            predictions[:, i] = (proba[:, 1] >= 0.5).astype(np.int64)
        else:
            predictions[:, i] = clf.predict(X_test)

        if (i + 1) % 100 == 0:
            print(f"  {i+1}/{len(label_cols)} ({time.time()-t0:.0f}s)")
    ...
    print(f"Saved {output_path}, shape={submission.shape}, time={time.time()-t0:.0f}s")
